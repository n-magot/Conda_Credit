...
def Generate_experimetnal_data(sample_size, intercept_Y, b_T_Y, b_Z2_Y,b_Z1_Y, random_seed=None):
    e = np.random.normal(size=sample_size)
    if random_seed is not None:
        np.random.seed(random_seed)

    # Generate independent binary variables T and Z2
    Z2 = np.random.binomial(1, 0.9, sample_size)  # Z2 ~ Bernoulli(0.7)
    Z1 = np.random.binomial(1, 0.4, sample_size)  # Z1 ~ Bernoulli(0.7)
    ...
    # Print diagnostics
    print(f"Experimental probabilities - Mean: {pr_Y.mean()}, Min: {pr_Y.min()}, Max: {pr_Y.max()}")
    return X_exp, Y


def Generate_observational_data(sample_size, intercept_Y, b_T_Y, b_Z2_Y, b_Z1_Y, intercept_T, b_Z2_T, random_seed=None):
    e = np.random.normal(size=sample_size)
    ...
    Z2 = np.random.binomial(1, 0.3, sample_size)  # Z2 ~ Bernoulli(0.3)
    Z1 = np.random.binomial(1, 0.4, sample_size)  # Z1 ~ Bernoulli(0.7)
    ...
    # Print diagnostics
    print(f"Observational probabilities - Mean: {pr_Y.mean()}, Min: {pr_Y.min()}, Max: {pr_Y.max()}")
    print(f"Confounder effect on T - Mean T given Z2=1: {np.mean(T[Z2 == 1])}, Z2=0: {np.mean(T[Z2 == 0])}")
    print(f"Confounder effect on Y - Mean Y given Z2=1: {np.mean(Y[Z2 == 1])}, Z2=0: {np.mean(Y[Z2 == 0])}")
    return X_obs, Y
...
n_runs = 1  # Number of different datasets that you want to test the algorithm
columns = []
...
for k in range(n_runs):
    ...
    # data_exp, labels_exp= Generate_experimetnal_data(Ne)
    # data_test, labels_test = Generate_experimetnal_data(N_test)
    # data_obs, labels_obs = Generate_observational_data(No)
    ...
    data_exp, labels_exp = Generate_experimetnal_data(
        sample_size=Ne,
    ...
    # Generate experimental data
    data_test, labels_test = Generate_experimetnal_data(
        sample_size=N_test,
        intercept_Y=intercept_Y,
        b_T_Y=b_T_Y,
        b_Z1_Y=b_Z1_Y,
    ...
    data_obs, labels_obs = Generate_observational_data(
        sample_size=No,
        intercept_Y=intercept_Y,
        b_T_Y=b_T_Y,
        b_Z1_Y=b_Z1_Y,
