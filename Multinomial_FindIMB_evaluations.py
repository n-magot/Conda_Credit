...
"""For 3 categories in the outcome variable uncomment lines 16-78"""
...
"""For 3 categories in the outcome variable"""
...
"""Inputs for simulate data with 3 categories in the outcome variable"""
# coefs = np.array([
#     [3.0, -3.0, 2.5],
#     [4.5, 4.5, -3.0],
#     [3.6, -1.5, 3.0]
...
def log_loss_GT(X_test, y_test, coefs, intercept):
    """
    Computes log-loss for binary outcome using ground-truth coefficients.
    """
    # Compute logits and probabilities
    logits = X_test @ coefs + intercept  # shape (N,)
    probs = expit(logits)                # probability of Y=1
...
def evaluate_expected_outcome_GT(X_test, coefs_GT, intercept_GT):
    """
    Calculate expected fraction of Y=1 under optimal treatment assignment
    for the ground-truth model.
    """
    N = X_test.shape[0]

    # Split features into T=0 and T=1
    X_T0 = X_test.copy()
    X_T0[:, 0] = 0  # Set treatment column to 0
    X_T1 = X_test.copy()
    X_T1[:, 0] = 1  # Set treatment column to 1

    # Compute predicted probabilities of Y=1
    probs_T0 = expit(X_T0 @ coefs_GT + intercept_GT)
    probs_T1 = expit(X_T1 @ coefs_GT + intercept_GT)

    # Assign best treatment per test case
    sumY1 = 0.0
    T0Count, T1Count = 0, 0
    for i in range(N):
        if probs_T0[i] > probs_T1[i]:
            estimateY1 = probs_T0[i]
            T0Count += 1
        else:
            estimateY1 = probs_T1[i]
            T1Count += 1
        sumY1 += estimateY1

    avgY1 = sumY1 / N
    print(f"Ground-truth: T0 chosen {T0Count}, T1 chosen {T1Count}")
    print(f"Expected fraction of Y=1 (avgY1) under GT optimal policy: {avgY1:.4f}")
    return avgY1
...
# ============================================================
# Core evaluation loop (shared across both modes)
# ============================================================

def evaluate_expected_outcome(Test_data, prob_provider, **provider_kwargs):
    probs_T0, probs_T1 = prob_provider(Test_data=Test_data, **provider_kwargs)
    N = len(probs_T0)

    sumY1, T0Count, T1Count = 0.0, 0, 0
    for i in range(N):
        if probs_T0[i] > probs_T1[i]:
            estimateY1 = probs_T0[i]
            T0Count += 1
...
De = simulate_binary_logit_exp(n_samples=Ne, coefs=coefs, intercepts=intercepts)
De_test = simulate_binary_logit_exp(n_samples=Ntest, coefs=coefs, intercepts=intercepts)

# Do.to_csv("/home/nandia/PycharmProjects/Conda_CREDIT/share_data_with_Sofia/Do.csv", index=False)
# De.to_csv("/home/nandia/PycharmProjects/Conda_CREDIT/share_data_with_Sofia/De.csv", index=False)
# De_test.to_csv("/home/nandia/PycharmProjects/Conda_CREDIT/share_data_with_Sofia/Dtest.csv", index=False)

# Do = pd.read_csv("/home/nandia/PycharmProjects/Conda_CREDIT/share_data_with_Sofia/Do.csv")
# De = pd.read_csv("/home/nandia/PycharmProjects/Conda_CREDIT/share_data_with_Sofia/De.csv")
# De_test = pd.read_csv("/home/nandia/PycharmProjects/Conda_CREDIT/share_data_with_Sofia/Dtest.csv")
...
for Z_cols in subsets:
    ...
    T0Count_alg, T1Count_alg, avgY1_alg = evaluate_expected_outcome(
        De_test,
        dirichlet_prob_provider,
        probs_matrix=P_Y_alg
    )
    ...
    if Z_cols == FS:
        ...
        T0Count_exp, T1Count_exp, avgY1_exp = evaluate_expected_outcome(
            De_test,
            dirichlet_prob_provider,
            probs_matrix=P_Y_exp
        )

        T0Count_obs, T1Count_obs, avgY1_obs = evaluate_expected_outcome(
            De_test,
            dirichlet_prob_provider,
            probs_matrix=P_Y_obs
        )


print('For algorithm:')
print(f"T0 chosen {T0Count_alg} times, T1 chosen {T1Count_alg} times")
print(f"Estimated fraction of Y=1 under best treatment policy: {avgY1_alg:.4f}")
print('For experimental')
print(f"T0 chosen {T0Count_exp} times, T1 chosen {T1Count_exp} times")
print(f"Estimated fraction of Y=1 under best treatment policy: {avgY1_exp:.4f}")
print('For observational')
print(f"T0 chosen {T0Count_obs} times, T1 chosen {T1Count_obs} times")
print(f"Estimated fraction of Y=1 under best treatment policy: {avgY1_obs:.4f}")
...
print('Expected Utility algorithm:', avgY1_alg)
print('Expected Utility experimental:', avgY1_exp)
print('Expected Utility observational:', avgY1_exp)

X_test_array = De_test[covariate_with_T].to_numpy()  # convert to numpy
avgY1_GT = evaluate_expected_outcome_GT(X_test_array, coefs_GT, intercepts_GT)
