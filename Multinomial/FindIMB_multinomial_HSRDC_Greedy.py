...
the UAI paper), and for the other estimators, we score based on the  marginal probability of Do, De, and (Do+De) for each set: P(Do|Z), P(De|Z)
and P(Do, De|Z) (Eq.5 for HZ_not_c from UAI paper, second row of the table S1 adjusted)
...
def compute_posterior_predictive_single(N_o_jk, N_o_j, N_e_jk, N_e_j, alpha_jk, alpha_j):
    """
    Returns:
    - probs_HZc: P(Y|do(X), Z, HZc) predictive probs under H_Z^c
    - probs_HZc_bar: P(Y|do(X), Z, HZc_bar) predictive probs under H̄_Z^c
    ...
    return probs_Y_HZc_bar, probs_Y_obs
...
def evaluate_expected_outcome(De_test, Do, De, df_scores, subsets, treatment, outcome, best_set_De, best_set_Do):
    ...
    def compute_BMA_probs(De_input):
        ...
        probs_Y_HZc_bar, probs_Y_obs_tmp = compute_posterior_predictive_single(
            N_o_jk, N_o_j, N_e_jk, N_e_j, alpha_jk, alpha_j
        )
        ...
        return P_Y_alg, P_Y_exp, P_Y_obs

    # Compute BMA probabilities for T0 and T1
    P_Y_alg_T0, P_Y_exp_T0, P_Y_obs_T0 = compute_BMA_probs(Dtest_T0)

    P_Y_alg_T1, P_Y_exp_T1, P_Y_obs_T1 = compute_BMA_probs(Dtest_T1)
    ...
    return T0_obs, T1_obs, avgY1_obs, T0_exp, T1_exp, avgY1_exp,  T0_alg, T1_alg, avgY1_alg
...
def bma_predict_and_evaluate(Do, De, De_test, df_scores, treatment, outcome, best_set_De, best_set_Do):
    """
    Perform BMA over subsets and evaluate metrics using the new expected utility function.
    """
    # Identify the full set of covariates
    ...
    T0_obs, T1_obs, avgY1_obs, T0_exp, T1_exp, avgY1_exp,  T0_alg, T1_alg, avgY1_alg = evaluate_expected_outcome(
        De_test=De_test,
        Do=Do,
        De=De,
        df_scores=df_scores,
...
for k in range(n_runs):
    ...
    Do = pre_pross_data(obs_data) #load your observational dataset
    De_all = pre_pross_data(exp_data) #load your experiemtnal dataset

    De, De_test = train_test_split(De_all, test_size=0.3, random_state=42)
    
    treatment = '' # put the name of your treatment variable
    outcome = ''  # put the name of your outcome variable
    ...
    # 1) Run layered search (or exhaustive, both produce the same df_results now)
